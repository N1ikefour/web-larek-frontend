...
events.on('basket:change', () => {
	...
	basket.items = appData.basket.itemlist.map((id, index) => {
		...
		const itemIndexElement = renderedCard.querySelector('.basket__item-index');
		if (itemIndexElement) {
			itemIndexElement.textContent = (index + 1).toString();
		}
