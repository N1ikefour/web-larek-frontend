...
export class Order extends Form<OrderForm> {
	...
	constructor(events: EventEmitter, container: HTMLFormElement) {
		...
		this._payCard = ensureElement<HTMLButtonElement>(
			'.button_alt[name=card]',
			this.container
		);
		this._payCash = ensureElement<HTMLButtonElement>(
	...
	set payment(value: Payment) {
		this.toggleClass('button_alt-active', value === 'card', this._payCard);
		this.toggleClass('button_alt-active', value === 'cash', this._payCash);
	}
